  ...
  render() {
    ...
    if (diff < 0) { // past date
      return (
        <span className="odometer-block">
          <p className='seedsale-text'>Public Sale Open</p>
        </span>
      );
    }
    ...
    return (
      <span className="odometer-block" align={'center'}>
        <p className='seedsale-text'>Public Sale starts in:</p>
        { units.map((unit, key) => {
          if (significance.indexOf(unit) !== -1) {
            return (
              <span key={key}  className="custom-ctt-numbers">
                <span ref={unit} className={`${unit} custom-ctt-numbers2`}>{this.dissect(state[unit], unit)}</span>
                {' '}
